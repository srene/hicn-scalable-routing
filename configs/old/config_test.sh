...
$VPPCTL -s $CLISOCK3 create interface memif id 0 socket-id 0 slave
sleep 1
$VPPCTL -s $CLISOCK3 set interface ip address memif0/0 2000::2/64
$VPPCTL -s $CLISOCK3 set int state memif0/0 up
$VPPCTL -s $CLISOCK3 create memif socket id 1 filename /run/vpp/memif1.sock
sleep 1
$VPPCTL -s $CLISOCK3 create interface memif id 0 socket-id 1 master
$VPPCTL -s $CLISOCK3 set interface ip address memif1/0 2001::1/64
$VPPCTL -s $CLISOCK3 set int state memif1/0 up


$VPPCTL -s $CLISOCK7 create memif socket id 1 filename /run/vpp/memif1.sock
sleep 1
$VPPCTL -s $CLISOCK7 create interface memif id 0 socket-id 1 slave
$VPPCTL -s $CLISOCK7 set interface ip address memif1/0 2001::2/64
$VPPCTL -s $CLISOCK7 set int state memif1/0 up
...
$VPPCTL -s $CLISOCK3 ip route add b001::/64 via 2001::2 memif1/0
