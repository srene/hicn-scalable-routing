...
$VPPCTL -s $CLISOCK6 create memif socket id 4 filename /run/vpp/memif4.sock
sleep 1
$VPPCTL -s $CLISOCK6 create interface memif id 0 socket-id 4 slave
$VPPCTL -s $CLISOCK6 set interface ip address memif4/0 2004::2/64
$VPPCTL -s $CLISOCK6 set int state memif4/0 up
$VPPCTL -s $CLISOCK6 create memif socket id 6 filename /run/vpp/memif6.sock
sleep 1
$VPPCTL -s $CLISOCK6 create interface memif id 0 socket-id 6 master
$VPPCTL -s $CLISOCK6 set interface ip address memif6/0 2006::1/64
$VPPCTL -s $CLISOCK6 set int state memif6/0 up

$VPPCTL -s $CLISOCK7 create memif socket id 5 filename /run/vpp/memif5.sock
sleep 1
$VPPCTL -s $CLISOCK7 create interface memif id 0 socket-id 5 slave
$VPPCTL -s $CLISOCK7 set interface ip address memif5/0 2005::2/64
$VPPCTL -s $CLISOCK7 set int state memif5/0 up
$VPPCTL -s $CLISOCK7 create memif socket id 6 filename /run/vpp/memif6.sock
sleep 1
$VPPCTL -s $CLISOCK7 create interface memif id 0 socket-id 6 slave
$VPPCTL -s $CLISOCK7 set interface ip address memif6/0 2006::2/64
$VPPCTL -s $CLISOCK7 set int state memif6/0 up
...
$VPPCTL -s $CLISOCK7 hicn pgen server name b001::1/64 intfc memif5/0
