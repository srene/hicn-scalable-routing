...
cat > /etc/vpp/config_prod.txt << EOL
create memif socket id 1 filename /run/vpp/memif1.sock
create interface memif id 1 socket-id 1 slave
set interface ip address memif1/1 fd01::2/64 
set int state memif1/1 up
ip route add fd00::0/64 table 10 via fd01::1 memif1/1
hicn enable b001::/64
EOL
...
typeset -i cnt=60
until ls -l /run/vpp/cli-vpp_prod.sock ; do
       ((cnt=cnt-1)) || exit 1
       sleep 1
done
